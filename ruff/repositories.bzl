...
load("//ruff/private:toolchains_repo.bzl", "PLATFORMS", "toolchains_repo")
...
def _ruff_repo_impl(repository_ctx):
    url = "https://github.com/someorg/someproject/releases/download/v{0}/ruff-{1}.zip".format(
        repository_ctx.attr.ruff_version,
...
ruff_toolchain(
    name = "ruff_toolchain",
    target_tool = select({
        "@bazel_tools//src/conditions:host_windows": "ruff_tool.exe",
        "//conditions:default": "ruff_tool",
    }),
)
"""
...
def ruff_register_toolchains(name, register = True, **kwargs):
    ...
    Args:
        name: base name for all created repos, like "ruff1_14"
